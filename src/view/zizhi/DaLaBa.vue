...
export default {
    ...
    methods: {
        showDetail: function (e) {
            var i = 0;
            if (i == 1) {
                this.$message(this.extraInfo[e.rowIndex]);
            }
        },
        fillItem(fill) {
            //从fill中提取部分数据作为item列元素
            var item = [];
            item.push(fill.publisher + ":" + fill.content);
            item.push(
                '<li class="iconfont icon-yangshengqi" style="list-style-type: none;"></li>'
            );
            this.extraInfo.push(fill.content);
            return item;
        },
        getItems() {
            this.$axios.get("/broadcast/getAll_on_app").then((res) => {
                ...
                if (configData.length % 2 == 1) {
                    //确保拥有偶数条记录使得奇偶背景色总是交替出现
                    i += 1;
